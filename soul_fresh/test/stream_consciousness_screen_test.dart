...
void main() {
  testWidgets('Stream of Consciousness timer start, pause, resume', (tester) async {
    // Render inside a scrollable oversized container to avoid vertical overflow in test constraints.
    await tester.pumpWidget(const MaterialApp(
      home: Scaffold(
        body: SingleChildScrollView(
          child: SizedBox(height: 1200, child: StreamConsciousnessScreen()),
        ),
      ),
    ));
  ...
  await tester.pump();
    final pausedSnapshot = tester.widgetList<Text>(find.byType(Text)).map((t) => t.data).join();

  // Advance time further; should not decrement while paused
  await tester.pump(const Duration(seconds: 3));
    final afterPauseSnapshot = tester.widgetList<Text>(find.byType(Text)).map((t) => t.data).join();
  ...
  await tester.pump(const Duration(seconds: 2));
    ...
    expect(textsAfterResume.any((s) => RegExp(r'09:\d{2}').hasMatch(s)), isTrue);
  });

  testWidgets('Stream of Consciousness completes exercise after writing', (tester) async {
    await tester.pumpWidget(const MaterialApp(
      home: Scaffold(
        body: SingleChildScrollView(
          child: SizedBox(height: 1200, child: StreamConsciousnessScreen()),
    ...
    await tester.enterText(writingField, 'This is my stream of consciousness writing for testing purposes.');
    await tester.pump();
